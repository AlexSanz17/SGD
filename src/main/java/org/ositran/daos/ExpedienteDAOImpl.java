...
import org.ositran.ajax.autocomplete.toolDwr;
...
import com.btg.ositran.siged.domain.Numeracionexpediente;
...
public class ExpedienteDAOImpl implements ExpedienteDAO{
	...
	public void aplicarNumeracionInternaExpediente(Integer iIdExp){
		...
		Proceso proceso= null;//data.getProceso();
	...
	public List<Map<String, String>> buscarLstPorCliente(Integer iIdCliente,Character cEstado){
	    log.debug("-> [DAO] ExpedienteDAO - buscarLstPorCliente():List<Expediente> ");
            List<Map<String, String>> l1 = null;
             
	     String  sql =  " select distinct e.idExpediente, e.nroExpediente, e.cliente, e.asunto, e.observacion, decode(c.codtipoinstitucion,3, "  +
                               " c.nombres || ' ' || c.apellidoPaterno || ' ' || c.apellidoMaterno, c.razonsocial) razonsocial , e.serie " +
                               " from expediente e, cliente c, documento d where c.idCliente = :idCliente and c.idCliente = d.id_cliente and e.idExpediente = d.expediente and e.estado =:estado and d.estado not in ('I','N') " ;
                             //  " e.proceso = p.idProceso ";
             
             l1 = new LinkedList<Map<String, String>>();   
             Query q = em.createNativeQuery(sql.toString());
             q.setParameter("idCliente", iIdCliente).setParameter("estado", cEstado);
		...
		Object[] objectArray = (Object[]) obj;
                ...
                datos.put("nroexpediente", objectArray[1].toString());
                datos.put("idcliente", objectArray[2].toString());
                datos.put("asunto", objectArray[3]==null?"": objectArray[3].toString().trim());
                datos.put("observaciones", objectArray[4]==null?"":objectArray[4].toString().trim());
                datos.put("razonsocial", objectArray[5]==null?"":  objectArray[5].toString().trim());
                datos.put("serie", objectArray[6].toString());
                l1.add(datos);
	     }
   ...
   public Integer generateNroExpedienteDesarrollo() {
	...
	public List<Expediente> findByCriteria(String numeroIdentificacion,String razonSocial,String numeroExpediente,String asunto,int proceso,Date fecha, String tipoBusqueda, Integer idusuario, String nt){
	     log.debug("-> [DAO] ExpedienteDAO - findByCriteria():List<Expediente> ");
             ...
             try{   
                String query="SELECT e FROM Expediente e ";// , Documento d WHERE 1 = 1 and e.id = d.expediente " ;
                String where = " WHERE 1 = 1 ";
                
                if(!StringUtils.isEmpty(numeroExpediente)){
                    	where += " AND e.nroexpediente = :nroexpediente ";
		}
		if(!StringUtils.isEmpty(asunto)){
                    	where += " AND LOWER(e.asunto) LIKE :asunto ";
		}
                if(!StringUtils.isEmpty(nt)){
                        query += ", Documento d ";
			where += " AND d.ID_CODIGO = :nt AND d.expediente = e.id  and d.documentoreferencia is null ";
		}
                
                where+=" AND e.estado='A' ORDER BY nroexpediente ";
		
                q=em.createQuery(query + where);
		...
		if(!StringUtils.isEmpty(asunto)){
                ...
                if(!StringUtils.isEmpty(nt)){
                	q.setParameter("nt", new Integer(nt));
		}
             }catch(Exception e){
                 e.printStackTrace();
             }
