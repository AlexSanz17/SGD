...
<html>
   <head>
      <title>Mantenimiento de Expedientes</title>
      ...
      <style type="text/css">
         ...
         #myGrid .aw-column-5 {width: 200px;text-align: center;}
      ...
      <script type="text/javascript">
         ...
         var mydata=[<%
            ...
            for (int i = 0; i < lstDocumento.size(); i++) {
               objDocumento = (Documento) lstDocumento.get(i);  
               %>["<%=objDocumento.getExpediente().getIdexpediente()%>","<%=objDocumento.getExpediente().getNroexpediente()%>","<%=objDocumento.getExpediente().getProceso().getNombre()%>","<%=((objDocumento.getExpediente()!=null&&objDocumento.getExpediente().getCliente()!=null&&objDocumento.getExpediente().getCliente().getRazonSocial()!=null)?objDocumento.getExpediente().getCliente().getRazonSocial():objDocumento.getExpediente().getCliente().getNombres()+" "+objDocumento.getExpediente().getCliente().getApellidoPaterno())%>","<%=objDocumento.getFechaCreacion() %>"],<%
            }
         %>]
      ...
      <script type="text/javascript">
         ...
         obj.setCellFormat([num, str, str, str, str]);
         //	provide cells and headers text
         obj.setCellText(mydata);
         obj.setHeaderText(myColumns);
         //	set number of rows/columns
         obj.setRowCount(<%=lstDocumento.size()%>);
         ...
         obj.onCellClicked = function(event, col, row) {
            ...
            if (id!="")
            {
               A('#e1f3ff',id,'/siged/doViewExpediente.action')
            }
            else
            {
         ...
         <tr>
            <td height="20"colspan="6" class="titulo" width="99%">
               <table width="70%" border="0" height="20" align="left">
                  <tr>
                     <td bgcolor="#4481B8" width="30%" align="center" class="tituloRojo">Mantenimiento de Expedientes</td>
                     <td></td>
                  </tr>
               </table>
            </td>
